...
$("#search_by_text").click(function(){
	...
	limit_load=0;
		...
		var milestone=$('#myDropdown .dd-selected-value').val();
		ajaxindicatorstart('loading data.. please wait..');
		$url = '<?php echo base_url();?>index.php/order/recived_order_list?milestone='+milestone+'&text='+$value;
			$.ajax({ cache: false,
				url: $url,
				type:"get",
				success: function (data) {
					...
					Ladda.bind( 'button[type="submit"],input[type="submit"],#login,#register,.buttons-pdf,.nxt_click,.view_mlileston_detail,.prv_click,.product_dispatcher_status,.dispatcherList',{ timeout: 500 } );
				},
				complete: function(data) {
					ajaxindicatorstop();
				}
			...
			$(document).ready(function() {
					...
					$(document).on("click", ".view_mlileston_detail", function(){
						...
						if($('.'+order).hasClass("in" ))
						{	
							$('.'+order).removeClass('in');
							$('.'+order).addClass( "out" );
					...
					//	$(".collapse.in").removeClass('in out');
						...
						$(".inner-table-mileston").parent().parent().removeClass('in').addClass('out');
						...
						$.post("<?php echo base_url();?>index.php/order/getmilestone",{ order_id: order,mileston_id: mileston},
							function(data) 
							{
								...
								total_receive_amount +=(element['amount'] - element['tp_amount']);
								...
								table=table+"</td>";
								var html='';						
								var  received_am = (parseFloat(element['amount'] - element['tp_amount'])*parseFloat($c_value)).toFixed(2);
								
								var html1='';
								var received_am1='';
								if(element['milestone_status']=='2')
								{
									var commission=(parseFloat(element['cmp_commission']/100)).toFixed(2);
									commission=parseFloat(element['amount'])*commission;
									html1='<br><small class="text-danger">TDM Commission('+(parseFloat(element['cmp_commission'])).toFixed(0)+'%) : <?php echo $currency_symbol;?>'+commission+'</small>';
									received_am1 = (parseFloat(received_am) - parseFloat(commission*parseFloat($c_value))).toFixed(2);
									
								}	
								
								if(parseFloat(element['final_amount'])<parseFloat(element['amount']))
								{
									received_am = (parseFloat(element['final_amount']  - element['tp_amount'])*parseFloat($c_value)).toFixed(2);
									total_receive_amount = received_am;
									html+="<br><span class='text-danger' id=''>Amount Refunded  : <?php echo $currency_symbol;?>"+(parseFloat(element['amount']-element['final_amount'])*parseFloat($c_value)).toFixed(2)+"</span>";
									
								}
														
								table=table+"<td>";
								table=table+'<?php echo $currency_symbol;?>'+(parseFloat(element['amount'])*parseFloat($c_value)).toFixed(2)+'<br><small class="text-danger">TP Commission ('+element['tp_plan']+'%) '+'<?php echo $currency_symbol;?>'+(parseFloat(element['tp_amount'])*parseFloat($c_value)).toFixed(2)+'</small>'+html1+'<br><small class="text-primary">Receivable Amount: <?php echo $currency_symbol;?>'+received_am1+html+'</small>';
								...
								if(cntPage>0)
								{
									...
									table=table+"<td class='text-left' colspan='7'>";
									table=table+"<span class='payment-from'><span>Payment To : </span>"+parsed[0]['payee_name']+'@'+parsed[0]['company_url']+"</span> <span>Vendor ID/URL : </span> "+parsed[0]['order_key']+"/"+parsed[0]['company_url']+" &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Pending Amount : </span> <?php echo $currency_symbol;?>"+((parseFloat(parsed[0]['total_amount'] - amount)*parseFloat($c_value)).toFixed(2))+'<span>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Total Receivable Amount:  <?php echo $currency_symbol;?>'+total_receive_amount+'</span>';
									table=table+"</td>";									
									table=table+"</tr>";	
									table=table+"</table>";
									table1=table1+table;
