...
class Order extends CI_Controller{
	...
	{	
		//	echo $this->sms->send('You send money to contact info -><a> 7567442153 Key : 300616jaG8F044931','+917567442153');
	...
	public function milestone_payment(){
		...
		if($this->session->userdata('customer_id'))
		{
			$dat['data']='order/milestone_payment'.'?id='.urlencode($_GET['id']);
			$dat['session_id']=mt_rand(100000, 999999);
			$dat['date_added']=date('Y-m-d');
			$dat['customer_id']=$this->session->userdata('customer_id');
			$this->db->insert('session_temp',$dat);
			$this->session->set_userdata('session_temp_id',$dat['session_id']);
		}
		}
		if($this->input->server('REQUEST_METHOD')=='POST')
		{
	...
	public function payment(){
		...
		if($this->session->userdata('customer_id'))
		{
			$dat['data']='order/payment'.'?id='.urlencode($_GET['id']);
			$dat['session_id']=mt_rand(100000, 999999);
			$dat['date_added']=date('Y-m-d');
			$dat['customer_id']=$this->session->userdata('customer_id');
			$this->db->insert('session_temp',$dat);
			...
			if($this->input->post('first_milestone')!==FALSE)
			{
				...
				if ($this->form_validation->run() !== FALSE)
				{
					...
					else
					{
						...
						$this->session->set_userdata('bal',1); 
						redirect('order/payment?id='.urlencode($this->input->post('key')));
					}
				}
			}
			else
		...
		{
			if((isset($balance[0]) && $balance[0]['amount']>=($order['total_amount']*$currency_detail->value)))
			{
				$data['suffi_balance']=1;
			}
			else
		...
		/* if(isset($order['payee_key']) && $order['payee_key']!='' )
		{
			$order['payee_key']
		} */
		...
		{	
			$pay_id='1';
			$data["supplier_name"]=isset($payee[0]['first_name'])?$payee[0]['first_name']:''." ".isset($payee[0]['last_name'])?$payee[0]['last_name']:'';
		}
		if($pay_id==0)
		{
			$to = $company[0]['email'];
			...
			$subject='Seller Missing Trustedpayer Key';
			$message='Seller / Service Provider does not Registered with TrustedPayer or does not connected with Trusteddealmaker';
			mail($to, $subject, urldecode($message), $headers);
		}
		$data["pay_id"]=$pay_id;
		$data['company_logo']=$company[0]['photo'];
		$data['company_name']=$company[0]['company_name'];
		...
		$data['current_url']=$order['current_url'];
	...
	public function order_payment(){	
		...
		if($this->session->userdata('customer_id'))
		{
			$dat['data']='order/order_payment'.'?id='.urlencode($_GET['id']);
			$dat['session_id']=mt_rand(100000, 999999);
			$dat['date_added']=date('Y-m-d');
			$dat['customer_id']=$this->session->userdata('customer_id');
			$this->db->insert('session_temp',$dat);
		...
		if($this->input->server('REQUEST_METHOD')=='POST')
		{			
				$this->session->unset_userdata('de_trans');
		...
		if((isset($balance[0]) && $balance[0]['amount']>=($data['total_amount'])))
		{
			$data['suffi_balance']=1;
	...
	public function get_product_api($key){
		...
		$config['redirect_success_url'] = $data['sucess_redirect_url']."?txn=1&".$fields_string;
		$da=array('redirect_success_url'=>$data['sucess_redirect_url']."?txn=1&".$fields_string);
	...
	public function milestone_release()
	{
		...
		//	print_R($milestone);exit;
			...
			if(isset($valid[0]))
			{
				...
				redirect($milestone['sucess_redirect_url']."?txn=".$milestone['txn_id']);
			}
			else
			{
	...
	//	print_R($order);exit;
		...
		if($customer_id==$orders[0]['payer_id'])
		{
			...
			$data['company_logo']=isset($company[0]['photo'])?$company[0]['photo']:'';
			$data['company_name']=isset($company[0]['company_name'])?$company[0]['company_name']:'';
			$data['company_website']=isset($company[0]['company_website'])?$company[0]['company_website']:'';
			//$this->load->view('header',$data);
