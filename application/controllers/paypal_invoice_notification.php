...
class Paypal_invoice_notification extends CI_Controller{
		...
		if(isset($_GET['PayerID']))
		{
			...
			if($output['ACK']=="Success")
			{
				...
				else
				{
					//print_r($output1);
					//echo urldecode($result);
					$invoice_id=str_replace('Transfer_','',$output['PAYMENTREQUEST_0_PAYMENTREQUESTID']);
					//echo $customer_id;
				//die();
					//$json = $result;
				...
				//$customer_id=str_repeat('Transfer_','',$output['PAYMENTREQUEST_0_PAYMENTREQUESTID']);
						$this->load->model('invoice_model');
							...
							{
								$this->invoice_model->payInvoice($invoice_id,array('amount'=>$output1['PAYMENTINFO_0_AMT']));
								$redr_data=$this->invoice_model->getInvoice($invoice_id);
								
								$headers  = 'MIME-Version: 1.0' . "\r\n";
								$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
								$headers .= 'From: Trustedpayer <info@trustedpayer.com>' . "\r\n";
								$message = $this->load->view('invoice-paid-letter', $redr_data, true);
								
								$flag=mail($redr_data['invoice'][0]['send_to'], 'TrustedPayer Invoice', urldecode($message), $headers);
								
								redirect('invoice/pay/'.$redr_data['invoice'][0]['key']."?success=1");
							}
							else{
								echo "error";
							}
